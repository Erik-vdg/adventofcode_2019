...
class Point(object):
    ...
    def __mul__(self, other: Type[numbers.Number]):
        # Scalar Multiplication
        if not isinstance(other, numbers.Number):
            return NotImplemented
        return Point(self.x * other, self.y * other)

    def __rmul__(self, other: Type[numbers.Number]):
        # Scalar Multiplication
        if not isinstance(other, numbers.Number):
            return NotImplemented
        return Point(self.x * other, self.y * other)
    ...
    def on_segment(self, line_segment: "LineSegment") -> Optional["LineSegment"]:
        ...
        if isinstance(line_segment, LineSegment):
            if (line_segment.orientation == "V" and self.x == line_segment.point_1.x) or (
                line_segment.orientation == "H" and self.y == line_segment.point_1.y
            ):
...
class LineSegment(object):
    ...
    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}(point_1={repr(self.point_1)}, point_2={repr(self.point_2)})"
        )
    ...
    def intersects(self, other_linesegment: "LineSegment") -> Optional[Point]:
        # If this LineSegment intersects another LineSegment, return the point that they intersect.
        ...
        if self.orientation == "H" and other_linesegment.orientation == "V":
            if (
                (self.point_1.x < other_linesegment.point_1.x < self.point_2.x)
                or (self.point_1.x > other_linesegment.point_1.x > self.point_2.x)
            ) and (
                (other_linesegment.point_1.y < self.point_1.y < other_linesegment.point_2.y)
                or (other_linesegment.point_1.y > self.point_1.y > other_linesegment.point_2.y)
            ):
            ...
            if (
                (self.point_1.y < other_linesegment.point_1.y < self.point_2.y)
                or (self.point_1.y > other_linesegment.point_1.y > self.point_2.y)
            ) and (
                (other_linesegment.point_1.x < self.point_1.x < other_linesegment.point_2.x)
                or (other_linesegment.point_1.x > self.point_1.x > other_linesegment.point_2.x)
            ):
    ...
    def intersects(self, other: Union["Wire", LineSegment]):
        if isinstance(other, self.__class__):
            return [
                point
                for point in [
                    this_segment.intersects(other_segment)
